...
def get_data(rl,itr):
  x=np.loadtxt("data/xy/x"+str(rl)+"."+str(d)+".xy.dat")
  y=np.loadtxt("data/xy/y"+str(rl)+"."+str(d)+".xy.dat")
  rho=np.loadtxt("data/xy/rho"+str(rl)+"."+str(d)+".xy.dat")
  rho=np.log10(rho)
  return x,y,rho
...
for i,d in enumerate(itrList):
  ...
  ax[i].set_yticks([-60,-30,0,30,60])
  ax[i].set_yticklabels(["$-60$","$-30$","$0$","$30$","$60$"])
  ...
  if i==0:
    ax[i].annotate(r'$\mathbf{t = %6.2f\ [\mathbf{ms}]}$' % (time),xy=(0,60), color='white',size=8) 
  else: 
    ax[i].annotate(r'$\mathbf{t = %6.2f\ [\mathbf{ms}]}$' % (time),xy=(0,60), color='black',size=8) 
...
#
ax[0].set_ylabel(r'$y  \,\rm [km]$')
ax[0].set_xlabel(r'$x  \,\rm [km]$')
ax[1].set_xlabel(r'$x  \,\rm [km]$')
ax[2].set_xlabel(r'$x  \,\rm [km]$')
#
#
cb=fig.colorbar(cbar,ax=ax[0:3],ticks=[5.0,7.0,9.0,11.0,13.0,15.0],shrink=0.45,pad=0.02,aspect=20)
